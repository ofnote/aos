from aos.checker import has_shape
...
def checks():
    has_shape(d1, 'a.b.int')

def tfms():
    #select
    do_tfm(d1, 'a.b.v -> b.v') # {b: 1}

    do_tfm(d2, 'a & v -> v') #{b: 1, c: 2}

    do_tfm(d3, 'a & v -> x.y.v') #{x : {y: 1}}

    do_tfm(d4, 'a & (v)* -> x & f(v)', f=lambda v: v[0]) #{ "x": 1 }
    #todo: basic expressions: x & v[0]

    do_tfm(d4, 'a & (v)* -> (x & v)* ') # [{ "x": 1 }, { "x": 2 }]

    do_tfm(d2, 'a & (k & v)* -> ((x.k) | (y.v))*') # [{ "x": "b", "y": 1 }, { "x": "c", "y": 2 }]


    do_tfm(d5, '(x & v)* -> x & (v)*') #{ "x": [1, 2] }


    do_tfm(d6, '(x & v | z)* -> (z | (x & list(v))*)') #[{ "y": "a", "x": [1]}, { "y": "b", "x": [2]}]
    #simpler: x & (v)* ? overload (v)* accumulator as list constructor

    do_tfm(d7, 'a & (b & v)* -> (b & (v)*)') #{"b": [1, 2]}
